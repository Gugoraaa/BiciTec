...
export default function UsersTable({
  ...
  const getButtonClass = (status: UserStatus) => {
    const baseClass = 'px-3 sm:px-4 py-1.5 sm:py-2 rounded-lg border text-sm sm:text-base transition-all duration-200';
        ...
        <div className="overflow-x-auto">
          <div className="min-w-[800px] sm:min-w-0">
            ...
            <div className="grid grid-cols-12 gap-4 px-4 py-3 text-xs sm:text-sm text-slate-400 border-b border-slate-800 bg-slate-900/50 whitespace-nowrap">
              ...
              <div className="col-span-3 font-medium">{tColumns('lastName')}</div>
              <div className="col-span-2 font-medium">{tColumns('status')}</div>
              <div className="col-span-2 font-medium text-right">{tColumns('creationDate')}</div>
            </div>
            ...
            <ul className="divide-y divide-slate-800 min-h-[200px]">
              {isLoading ? (
                ...
                <li className="text-center py-8 text-slate-400 px-4">{t('noUsers')}</li>
              ) : (
                filteredUsers.map((u) => (
                  <li 
                    key={u.id} 
                  ...
                  >
                    <div className="col-span-2">
                      <div className="font-medium text-slate-100 text-sm sm:text-base truncate" title={u.studentId}>
                        {u.studentId}
                      </div>
                    </div>
                    <div className="col-span-3">
                      <div className="text-slate-300 text-sm sm:text-base truncate" title={u.name}>
                        {u.name}
                      </div>
                    </div>
                    <div className="col-span-3">
                      <div className="text-slate-300 text-sm sm:text-base truncate" title={u.lastName}>
                        {u.lastName}
                      </div>
                    </div>
                    <div className="col-span-2">
