    ...
    try {
      ...
      if (!Array.isArray(stationsData)) {
        if (typeof stationsData === "object" && stationsData !== null) {
          ...
          throw new Error(
            "Invalid data format received: Expected an array of stations"
          );
        }
      }
          ...
          {stations.length === 0 ? (
            ...
            stations.map((station, index) => {
              ...
              const displayValue = Math.min(
                100,
                Math.max(0, station.porcentaje_ocupacion)
              );
              const animatedWidth = displayValue * progress;

              return (
                <div key={station.id_estacion} className="space-y-2">
                  <div className="flex justify-between items-center">
                    <div className="flex items-center gap-2">
                      <span
                        className={`font-medium ${
                          isOffline ? "text-red-400" : "text-gray-300"
                        }`}
                      >
                        {station.nombre}
                      </span>
                      {isOffline && (
                        <span className="text-xs text-red-400 flex items-center gap-1 bg-red-500/10 px-2 py-0.5 rounded-full">
                          <FaExclamationCircle />
                          Offline
                        </span>
                      )}
                    ...
                    >
                      {isOffline
                        ? "N/A"
                        : `${station.bicicletas_disponibles}/${station.capacidad_total}`}
                    </span>
                  </div>
